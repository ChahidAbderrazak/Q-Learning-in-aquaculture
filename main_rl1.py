...
RESET_RL=0                                    # 0) load trained q_table, 1) train RL
...
for alpha in [ 0.5]:                         # learning rate  ~0.05
    for gamma in [ 0.5]:                      # discount factor ~0.5
        for reward_ in [1]:#range(5,7):
            for ZH_ in range(0,1):
                # for eps_ in [1/400]:                  # greedy police~1/explotion episodes
                ###################################################################################################################
                # Q-learning parameters
                N_ACTIONS=6                                 # ~3 the number of actions
                WEIGHT_RESOLUTION=10                        # ~5 Discretiztion grid of the states (weight)
                TIME_RESOLUTION=10                           # ~2 Discretiztion grid of the time (t)
                EPSILON = 0.9                               # ~0.9 greedy police: 0) totally random    1) follows the policy
                ...
                POLICY_ERROR= 0.7#1.2                           # ~1 means we stop if the policy did not change after POLICY_ERROR*N_STATES iteration
                ...
                eps_    = 1/(1000)#N_STATES                 # ~<1/(N_ACTIONS*N_STATES) the simulated annealing  factor
                MAX_FEED_RATIO=Fmax                          # The maximun tolerated feed ratio to the fish weight
                MIN_FEED_RATIO=Fmin
                FEEDING=[i for i in np.linspace(MIN_FEED_RATIO, MAX_FEED_RATIO, num=N_ACTIONS)]   ## 'R': feed with amount equal to k*W where W is the fish weight,
                ACTIONS = ['R='+str(i)+'*W' for i in FEEDING]
                MAX_EPISODES = -1#10*N_STATES*len(ACTIONS)     # maximum episodes
                ...
                # print('FEEDING=\n',FEEDING);print('W_STATES=\n',W_STATES);print('TIME_ZONES=\n',TIME_ZONES);
                # input('flag')
                if ZH_==1:
                ...
                def plot_and_save_results(time, food, fish_weight, Temp, IMPROVE_POLICY, filename, suff):
                    ...
                    ax2 = ax.twinx()
                    lns3 = ax2.plot(time, Temp, '-g', label = 'Water temperature')
                    ...
                    ax.set_title('Controlled feeding')
                    ...
                    ax2.legend( lns, labs, loc=0)#"lower right")#, frameon=False)
                ...
                def Get_aquaium_state(W, W_cont, W_discrt, T, T_cont, T_discrt):
                ...
                def build_q_table(n_states, actions):
                    ...
                    filename='./q_table_weights.pickle'
                ...
                def get_env_feedback(S, A, day):
                    ...
                    # print('ACTIONS=', ACTIONS);print('A=', A)
                    f=FEEDING[ACTIONS.index(A)]
                    # print('T=', Temp);print('day=', day)
                    # input('dfdf')
                    ...
                    if D_ == TIME_ZONES-1 and W_ == W_STATES-1:   # terminate
                        S_ = 'terminal'
                        R=0#(1/(sigma*np.sqrt(2*np.pi)))*np.exp(-0.5*((x-xf)**2)/(sigma**2))
                ...
                def growth_model(x, f, T, tk):
                    ...
                    dx = Fish_Growth_Model(x, f, Temp_k, DO_k, UIA_k);
                    # print('x=',x);print('dx=',dx)
                    x= x+dx*dt
                    # print('Updated x=',x, ' x_float=',x_float, ' x_int=',x_int)
                ...
                def rl(N_STATES, ACTIONS):
                    ...
                    # #filename='./results/RL1_var_'+suff+'.pickle'
                    ...
                    while run_learning:#in range(MAX_EPISODES):
                        ...
                        if episode%EPISODES_EVAL==0:
                            ...
                            q_table = build_q_table(N_STATES, ACTIONS)
                            time, food, fish_weight, total_food = rl_evaluation(q_table)                 # run policy evaluation on the trained Q-learning table
                    ...
                    #     plt.ylabel('Policy error')
                    #
                    # plt.savefig('./results/iter_policy_eps'+str(episode)+suff+'.png', format='png', dpi=1200)
                    # # plt.show()
                ...
                def rl_evaluation(q_table):
                    ...
                    while not is_terminated:
                        ...
                        time.append(t)
                        food.append(FEEDING[ACTIONS.index(A)])
                        ...
                        total_food=dt*np.sum(R_max*(np.multiply(food,fish_weight)))
                    return  np.asarray(time), np.asarray(food), np.asarray(fish_weight), total_food
                ...
                print('x=',x)
                ##################################################################################################################
                filename='./results/RL1_'
                #TAGGING the experiment
                suff='_r'+str(reward_)+'_grdy'+str(eps_)+'_alph'+str(alpha)+'_gamma'+str(gamma)+'_zh'+str(ZH_)+'_Lz'+str(L_ZH)+'_x0_'+str(x0)+'_xf'+str(xf)#+'_RMSE'+str(RMSE)    # tagging  the experiemnt

                filename_q_table='./q_table_weights.pickle'
                if RESET_RL==1 and os.path.isfile(filename_q_table):
                    os.remove('q_table_weights.pickle')
                #%%  Run the environment
