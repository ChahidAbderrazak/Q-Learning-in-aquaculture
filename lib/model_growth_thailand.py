...
def create_tank_env(N):
    for i in range(N):
        Temp = Topt+0.0*np.random.uniform(low=Tmin, high=Tmax, size=(N))
...
def get_actions2(FEEDING, HEATING):
    ACTIONS_value = list(itertools.product(*[FEEDING, HEATING]))
    ACTIONS = ['(R='+str(i[0])+'*W , T=' + str(i[1])+ 'C)' for i in ACTIONS_value]
    # print(ACTIONS_names)
...
filename="./data/Experimetal_data_Tilapia_Thailand_processed_days.csv"
